...
FilterManager::FilterManager(Engine *engine) {
    ...
    if (clProcessor->init()) {
        processor = clProcessor;
        LOGD("use cl processor");
    } else {
        delete clProcessor;
        clProcessor = nullptr;
        processor = new CpuProcessor();
        LOGD("use cpu processor");
    }
}
...
void FilterManager::renderFilterEffect(int filterTag, uint8_t * rgba, int width, int height, uint8_t *renderedRgba) {
    processor->setBufferSize(width * height * 4);
    if (filterTag == static_cast<int>(FilterTag::ORIGINAL)) {
        ...
        processor->process(static_cast<FilterTag>(filterTag), rgba, width, height, renderedRgba);
        ...
        delete[] yuvlut;
#endif
        processor->process(FilterTag::LUT_FILTER, rgba, width, height, lutTables[filterTag], lutWidth * lutHeight * 4,
                        renderedRgba);
    } else {
